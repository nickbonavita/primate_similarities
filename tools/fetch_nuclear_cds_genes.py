#!/usr/bin/env python3
"""Fetch nuclear CDS genes for all primates in this project.

Genes fetched:
- RAG1 (recombination activating 1)
- IRBP/RBP3 (interphotoreceptor retinoid-binding protein)
...
GENES = {
    "rag1": {
        "label": "nuclear RAG1 coding sequence",
        "aliases": ("gene=rag1", "recombination activating 1"),
        "header_terms": ("rag1", "recombination activating 1"),
        "terms": (
            "RAG1[Gene] AND mRNA",
            "recombination activating 1[Title] AND mRNA",
            "RAG1[Title] AND partial sequence",
            "RAG1[Gene]",
        ),
        "min_len": 900,
    },
    "irbp": {
        "label": "nuclear IRBP (RBP3) coding sequence",
        "aliases": (
            "gene=rbp3",
            "gene=irbp",
        ...
        "terms": (
            ...
            "IRBP[Title]",
            "irbp gene[Title]",
            "interphotoreceptor retinoid-binding protein[Title] AND mRNA",
            "interphotoreceptor retinoid-binding protein[Title] AND partial sequence",
            "interphotoreceptor retinoid binding protein[Title]",
            "RBP3[Gene]",
        ),
        "min_len": 900,
    },
...
def pick_fasta_gene_region(fasta_text: str, header_terms: tuple[str, ...], min_len: int) -> str | None:
...
def fetch_gene_for_species(species_name: str, gene_key: str, gene_info: dict) -> tuple[str, str]:
    all_ids = []
    ...
    if not all_ids:
        raise RuntimeError(f"No nucleotide candidates found for {species_name} {gene_key.upper()}")
    ...
    for nuccore_id in all_ids:
        ...
        # Some records are partial gene regions without explicit CDS features.
        fasta_text = fetch_fasta(nuccore_id)
        seq = pick_fasta_gene_region(fasta_text, gene_info["header_terms"], gene_info["min_len"])
        if seq:
            return nuccore_id, seq

        time.sleep(0.2)

    raise RuntimeError(f"Could not extract {gene_key.upper()} CDS for {species_name}")
...
def main() -> None:
    ...
    for species_id, species_name in SPECIES:
        for gene_key, gene_info in GENES.items():
            nuccore_id, seq = fetch_gene_for_species(species_name, gene_key, gene_info)
